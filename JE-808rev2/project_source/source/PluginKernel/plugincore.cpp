...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(0, "drivedB", "dB", controlVariableType::kDouble, 0.000000, 96.000000, 48.000000, taper::kLinearTaper);
	piParam->setBoundVariable(&drivedB, boundVariableType::kDouble);
	addPluginParameter(piParam);

	piParam = new PluginParameter(1, "Tone", "Hz", controlVariableType::kDouble, 1000.000000, 8000.000000, 3000.000000, taper::kVoltOctaveTaper);
	piParam->setBoundVariable(&Tone, boundVariableType::kDouble);
	addPluginParameter(piParam);

	piParam = new PluginParameter(2, "leveldB", "dB", controlVariableType::kDouble, -60.000000, 0.000000, -30.000000, taper::kLinearTaper);
	piParam->setBoundVariable(&leveldB, boundVariableType::kDouble);
	addPluginParameter(piParam);


	piParam = new PluginParameter(3, "bypass", "OFF, ON", "OFF");
...
void PluginCore::updateParameters() {
	...
	driveCooked = dB2Raw(drivedB);
	levelCooked = dB2Raw(leveldB);

	filterparams = FilterL.getParameters();

	filterparams.boostCut_dB = 0;
	filterparams.fc = Tone;
	...
	HPfilterparams = HPL.getParameters();

	filterparams.boostCut_dB = 0;
	filterparams.fc = 500.000000;
	filterparams.Q = 1;
	filterparams.algorithm = filterAlgorithm::kHPF2;

	HPL.setParameters(filterparams);
	HPR.setParameters(filterparams);
}
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFMono)
    {
		...
		double dry = channelData;

		channelData = HPL.processAudioSample(channelData);

		channelData = tanhWaveShaper(channelData, driveCooked) * 0.5;

		channelData = FilterL.processAudioSample(channelData);

		channelData = channelData * levelCooked;

		channelData = (channelData * bypass) + (dry * branchlessbypass[bypass]);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		//Save copies of dry signal
		double ldry = channelDataL;
		double rdry = channelDataR;

		//Apply HPF
		channelDataL = HPL.processAudioSample(channelDataL);
		channelDataR = HPR.processAudioSample(channelDataR);

		//Tan waveshaper to generate distortion
		channelDataL = tanhWaveShaper(channelDataL, driveCooked) * 0.5;
		channelDataR = tanhWaveShaper(channelDataR, driveCooked) * 0.5;

		//Apply LPF for tone effect
		channelDataL = FilterL.processAudioSample(channelDataL);
		channelDataR = FilterR.processAudioSample(channelDataR);

		//Adjust level with param scalar
		channelDataL = channelDataL * levelCooked;
		channelDataR = channelDataR * levelCooked;

		//Branchless bypass
		channelDataL = (channelDataL * bypass) + (ldry * branchlessbypass[bypass]);
		channelDataR = (channelDataR * bypass) + (rdry * branchlessbypass[bypass]);

		//Assign processed signal to output frame
